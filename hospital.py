...
class Hospital:
    ...
    def __create_db_if_not_exists(self):
        ...
        if not os.path.isfile('db.xlsx'):
            ...
            return xl.load_workbook('db.xlsx')
    ...
    def __authenticate(self, level, pwd):
        ...
        auth_sheet = self.workbook['Auth']
        return auth_sheet[level].value == pwd
        ...
        while True:
            if self.rule == 'admin':
                ...
                if x == '0':
                    ...
                    self.__show_action_menu('Patients')
                elif x == '2':
                    self.__show_action_menu('Doctors')
                elif x == '3':
                    self.__show_action_menu('Appointments')
                elif x == '4':
                    self.__show_action_menu('Departments')
                else:
                    print('\n*****Invalid input!*****\n')
            else:
                x = input('\n1: View patients\n2: View doctors\n'
    ...
    def __create_dependent_field_if_not_exists(self, title, data):
        ...
        if title == 'Doctors':
            ...
            for i in range(1, sheet.max_row+1):
                if sheet.cell(row=i, column=2).value == data[1]:
                    return True
            else:
                print('\n*****No department registerd with the name of '
                      + data[1]+'*****')
            ...
            for i in range(1, sheet.max_row+1):
                if sheet.cell(row=i, column=2).value == data[0]:
                    return True
            else:
                print('\n*****No doctor registerd with the name of '
                      + data[0]+'*****')
    ...
    def __add_record(self, title, data):
        ...
        if(title == 'Doctors' or title == 'Appointments'):
            if not self.__create_dependent_field_if_not_exists(title, data):
                return False
        ...
        for i in range(len(data)+1):
            sheet.cell(row=max_row+1, column=i+1).value = max_id + \
                1 if i == 0 else data[i-1]
    ...
    def __prompt_addition(self, title):
        ...
        if title == 'Patients':
            patient_name = self.__validate(
                'patient name',
                re.compile(r'^[a-zA-z]{1}.*'),
                '*****Name should start with character*****')
            patient_address = self.__validate(
                'patient address',
                re.compile(r'^[a-zA-z]{1}.*'),
                '*****IAddress should start with character*****')
            patient_age = self.__validate(
                'patient age',
                re.compile(r'\d+'),
                '*****Invalid Age! age must be a number*****')
            self.__add_record(
                title, [patient_name, patient_address, patient_age])
        elif title == 'Doctors':
            doctor_name = self.__validate(
                'doctor name',
                re.compile(r'^[a-zA-z]{1}.*'),
                '*****Name should start with character*****')
            department_name = self.__validate(
                'department',
                re.compile(r'^[a-zA-z]{1}.*'),
                '*****Department should start with character*****')
            self.__add_record(title, [doctor_name, department_name])
            ...
            self.__add_record(title, [doctor_name, patient_name, current_date])
        elif title == 'Departments':
            department_name = self.__validate(
                'department name',
                re.compile(r'^[a-zA-z]{1}.*'),
                '*****Name should start with character*****')
            self.__add_record(title, [department_name])

    def __view(self, title):
        """View all records of a specific sheet and 
        justifies rows/columns for a better view.
        """
        sheet = self.workbook[title]
        if sheet.max_row == 1:
        ...
        for i in range(1, sheet.max_column+1):
            ...
            for j in range(1, sheet.max_row+1):
                ...
                if rowValue == None:
                    break
                row.append(rowValue)
                if len(str(rowValue)) > rowLen:
                    rowLen = len(str(rowValue))
            else:
                items.append(row)
        ...
        while True:
            ...
            if x == 'q' or x == 'Q':
                ...
                while True:
                    ...
                    if self.__authenticate('B2', a):
                        self.rule = 'admin'
                        self.__show_admin_menu()
                        break
                    else:
                        print('\n*****Incorrect password!*****\n')
            elif x == '2':
